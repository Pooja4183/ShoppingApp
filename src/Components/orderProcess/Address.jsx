...
import { fetchAddressRequest } from "../../Redux/address/addressAction";
import { createOrderRequest } from "../../Redux/order/orderAction";
...
useEffect(() => {
  console.log("Effect fired:", success, order);
}, [success, order]);
  ...
  if (success===true && order?._id) {
    ...
    navigate(`/payment/${orderId}?totalAmount=${order.totalAmount}`);
    ...
    <div className="min-h-screen bg-white">
      <div className="flex justify-center mt-10 px-40">
              ...
              {otherAddresses.map((item) => (
                ...
                >
                  <div className="p-5">
                    ...
                    <p className="text-gray-600 text-sm">
                      {item.address}, {item.town_city}, {item.state} -{" "}
                      {item.pin_Code}
                    </p>

                    <p className="text-gray-700 text-sm mt-2">
                      Landmark:{" "}
                      <span className="font-semibold">{item.landmark}</span>
                    </p>

                    <p className="text-gray-700 text-sm mt-2">
                      Mobile:{" "}
                      <span className="font-semibold">{item.mobile}</span>
                    </p>
        ...
        <div className="w-2/5 border-l pl-10">
          ...
          <button
            type="button"
            className="mt-6 w-full py-3 rounded font-semibold transition bg-pink-600 text-white hover:bg-pink-700"
            onClick={() => {
